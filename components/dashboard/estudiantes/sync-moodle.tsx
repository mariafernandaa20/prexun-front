...
export default function SyncMoodle() {
  const [cohortStatus, setCohortStatus] = React.useState<
  ...
  const handleSyncMoodle = async () => {
    ...
    try {
      ...
      let errorMessage =
        'Hubo un problema al sincronizar los cohorts con Moodle.';
      if (error.response && error.response.data && error.response.data.errors) {
        const moodleErrors = error.response.data.errors;
        errorMessage =
          'Errores de Moodle (Cohorts):\n' + moodleErrors.join('\n');
      } else if (
        error.response &&
        error.response.data &&
  ...
  const handleSyncUsers = async () => {
    ...
    try {
      ...
      console.error(error);
      let errorMessage =
        'Hubo un problema al sincronizar los usuarios con Moodle.';
      if (error.response && error.response.data && error.response.data.errors) {
        const moodleErrors = error.response.data.errors;
        errorMessage =
          'Errores de Moodle (Usuarios):\n' + moodleErrors.join('\n');
      } else if (
        error.response &&
        error.response.data &&
        error.response.data.message
  ...
  const handleExportEmailAndGroup = async () => {
    try {
      ...
      toast({
        ...
        description:
          'El archivo CSV se ha generado y descargado correctamente.',
      });
    } catch (error) {
