...
const PaymentMethodBadge: React.FC<{ method: string }> = ({ method }) => {
  const methodConfig = {
    ...
    transfer: { label: "Transferencia", className: "bg-blue-50 text-blue-700 border-blue-200" },
    card: { label: "Tarjeta", className: "bg-purple-50 text-purple-700 border-purple-200" },
  }

  const config = methodConfig[method] || { label: method, className: "bg-gray-50 text-gray-700 border-gray-200" }

  return (
    <span className={`inline-flex px-2 py-1 rounded-md text-xs font-medium border ${config.className}`}>
      {config.label}
    </span>
  )
}
...
export default function AdeudosModal({ student }) {
  ...
  return (
    <>
      <Button variant="ghost" size="icon" title="Ver Adeudos" onClick={() => setOpen(true)} className="hover:bg-accent">
        ...
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-yellow-600">{overdueDebts}</div>
              </CardContent>
            </Card>
          </div>
              ...
              {debts.length > 0 ? (
                    ...
                    <TableBody>
                      {debts.map((debt) => (
                        <TableRow key={debt.id} className="hover:bg-muted/50">
                          ...
                          <TableCell>
                            <div className="space-y-1">
                              {debt.assignment?.period?.name && (
                                <div className="text-sm font-medium">{debt.assignment.period.name}</div>
                              )}
                              {debt.assignment?.grupo?.name && (
                                <div className="text-xs text-muted-foreground">Grupo: {debt.assignment.grupo.name}</div>
                              )}
                          ...
                          <TableCell className="text-right font-medium">{formatCurrency(debt.total_amount)}</TableCell>
                ...
                <Card>
                  <CardContent className="flex flex-col items-center justify-center py-12">
                    ...
                    <CardTitle className="text-lg mb-2">¡Sin adeudos!</CardTitle>
                    <CardDescription>Este estudiante no tiene adeudos pendientes</CardDescription>
                  </CardContent>
                </Card>
              )}
            </TabsContent>

            <TabsContent value="transactions" className="mt-4 overflow-auto flex-1">
              {transactions.length > 0 ? (
                      ...
                      {transactions.map((transaction) => (
                        <TableRow key={transaction.id} className="hover:bg-muted/50">
                          <TableCell className="font-mono text-sm">
                            {transaction.folio_new || transaction.folio || `#${transaction.id}`}
                          </TableCell>
                          <TableCell>
                            <PaymentMethodBadge method={getPaymentMethodLabel(transaction.payment_method)} />
                          </TableCell>
                          ...
                          <TableCell>
                            <StatusBadge status={transaction.paid !== 0 ? "paid" : "pending"} />
                          </TableCell>
                ...
                <Card>
                  <CardContent className="flex flex-col items-center justify-center py-12">
                    ...
                    <CardTitle className="text-lg mb-2">Sin transacciones</CardTitle>
                    <CardDescription>No hay transacciones registradas para este estudiante</CardDescription>
                  </CardContent>
                </Card>
              )}
            </TabsContent>
