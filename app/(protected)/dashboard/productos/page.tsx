...
export default function ProductsPage() {
    ...
    const [newProducto, setNewProducto] = useState<Producto>({ name: '', description: '', price: 0, stock: 0 });
                            ...
                            <TableRow key={producto.id}>
                                ...
                                <TableCell>
                                    {editingId === producto.id ? (
                                        <Input
                                            value={producto.name}
                                            onChange={(e) => setProductos(productos.map(p =>
                                                p.id === producto.id ? { ...p, name: e.target.value } : p
                                            ))}
                                        />
                                    ) : (
                                        producto.name
                                ...
                                <TableCell>
                                    {editingId === producto.id ? (
                                        <Input
                                            ...
                                            onChange={(e) => setProductos(productos.map(p =>
                                                p.id === producto.id ? { ...p, description: e.target.value } : p
                                            ))}
                                        />
                                    ) : (
                                ...
                                <TableCell>
                                    {editingId === producto.id ? (
                                        <Input
                                            ...
                                            onChange={(e) => setProductos(productos.map(p =>
                                                p.id === producto.id ? { ...p, price: Number(e.target.value) } : p
                                            ))}
                                        />
                                    ) : (
                                        `$${producto.price}`
                                ...
                                <TableCell>
                                    {editingId === producto.id ? (
                                        <Input
                                            ...
                                            onChange={(e) => setProductos(productos.map(p =>
                                                p.id === producto.id ? { ...p, stock: Number(e.target.value) } : p
                                            ))}
                                        />
                                    ) : (
                                        producto.stock
                                ...
                                <TableCell>
                                    {editingId === producto.id ? (
                                        <div className="flex gap-2">
                                            <Button onClick={() => handleUpdate(producto)}>Guardar</Button>
                                            <Button variant="outline" onClick={() => setEditingId(null)}>Cancelar</Button>
                                        </div>
                                    ) : (
                                        <div className="flex gap-2">
                                            <Button variant="outline" onClick={() => setEditingId(producto.id)}>Editar</Button>
